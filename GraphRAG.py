...
# Helper: Is this a question node?
def is_question(text):
    return "?" in text.strip()

# Track which nodes are questions
question_nodes = set()
for node in nodes:
    if is_question(node["text"]):
        question_nodes.add(node["id"])
...
for i in range(len(enriched_nodes) - 1):
    ...
    # ➤ Continuation ( speaker continuation, if same speaker sentence broken into 2 nodes )
    if curr["speaker"] == next_node["speaker"]:
        edge_cont = { "from": curr_id, "to": next_id, "type": "continuation" }
        edges.append(edge_cont)
        print(f"⏩ Edge (Continuation): {curr_id} → {next_id} (Same speaker)")
...
for i, curr in enumerate(nodes):
    ...
    if is_question(curr_text):
        ...
        # Look for a valid answer node (different or same speaker)
        for j in range(i + 1, len(nodes)):
            responder = nodes[j]
            responder_id = responder["id"]
            responder_speaker = responder["speaker"]

            if responder_id != curr_id:
                # ➤ Question edge (from question → answer)
                question_edge = {
                    "from": curr_id,
                    "to": responder_id,
                    "type": "question"
                }
                edges.append(question_edge)
                print(f"🧭 Question Edge: {question_edge['from']} → {question_edge['to']}")

                # ➤ Answer edge (from answer → question)
                answer_edge = {
                    "from": responder_id,
                    "to": curr_id,
                    "type": "answer"
...
# Related-topic edges using pairwise semantic similarity
...
print("------------------SUBGRAPH RETRIEVAL FOR EACH AUDIT CRITERIA PROCESS----------------")

#-------------------Subgraph Retrieval for each audit criteria------------------- 
def retrieve_subgraph_for_criterion(
    criterion: str,
    nodes: list,
    edges: list,
    embed_model,
...
for criterion in stage_1_criteria:
    result = retrieve_subgraph_for_criterion(criterion, nodes, edges, embed_model, min_score=0)
...
print("==================COMBINED RESULTS STAGE 1:===============\n")
...
# ------------------------------AUDITING RESULTS----------------------------------
print("==================AUDITING RESULTS STAGE 1:===============\n")
...
# Example: print top node IDs for first criterion
print("\n✅ Top Node IDs for First Criterion:")
for node in stage2_subgraphs[0]["top_nodes"]:
    print(f"- {node['id']}")
...
print("==================COMBINED RESULTS STAGE 2:===============\n")

print_combined_results(combined_result)

print("==================AUDITING RESULTS STAGE 2:===============\n")
